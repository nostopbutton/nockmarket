...
module.exports = {
    ...
    addStock: function(uid, stock, callback) {
        ...
        db.push('users', new ObjectID(uid), {portfolio: stock}
            , doCallback);
    },
    ...
    order.type = (Math.random() > 0.5 ? exchange.BUY : exchange.SELL);
...
function transformStockData(stock, existingData) {
    ...
    // we add some fillers for nonexistent values
    for (var i = 1; i <= 5; i++) {
        if (!newData['b' + i + 'p']) {
            newData['b' + i + 'p'] = 0;
            newData['b' + i + 'v'] = 0;
